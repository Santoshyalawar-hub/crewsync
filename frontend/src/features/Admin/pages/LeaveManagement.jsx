...
.lm-row:hover { border-color:rgba(255,107,53,.25); box-shadow:0 4px 16px rgba(13,31,45,.07); }

/* action buttons */
.lm-approve { display:inline-flex; align-items:center; gap:5px; padding:7px 14px; border-radius:9px; border:1.5px solid rgba(0,194,168,.3); background:rgba(0,194,168,.08); color:${T.teal}; font-size:11px; font-weight:700; font-family:'DM Sans',sans-serif; cursor:pointer; transition:all .15s; }
.lm-approve:hover { background:rgba(0,194,168,.15); border-color:${T.teal}; }
    ...
    <div className="lm" style={{padding:'0 0 56px'}}>
      ...
      <div style={{background:`linear-gradient(135deg,${T.navy},${T.navyMid})`,padding:'22px 26px',position:'relative',overflow:'hidden',marginBottom:22}}>
        ...
        <div style={{position:'relative',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
          <div>
            ...
            <h1 className="fd" style={{fontSize:23,fontWeight:900,color:'#fff',margin:0}}>Leave & Attendance Requests</h1>
            <p style={{fontSize:13,color:'rgba(255,255,255,.5)',marginTop:4}}>Review pending leave applications and attendance correction requests.</p>
          </div>
      ...
      <div style={{padding:'0 26px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:20}} className="lm-kpi">
          ...
          {[
            {emoji:'🗓️',cap:'Pending Leaves',     val:pending.leaveRequests.length,      accent:T.coral,  bg:'rgba(255,107,53,.08)'},
            {emoji:'⚙️',cap:'Pending Corrections', val:pending.correctionRequests.length, accent:T.teal,   bg:'rgba(0,194,168,.08)'},
          ].map(k=>(
        ...
        <div className="lm-grid">
          ...
          {/* ── LEAVE REQUESTS ── */}
          <SectionCard title="Pending Leave Requests" sub="Approve or reject employee leave applications" count={pending.leaveRequests.length} accent={T.coral} Icon={Calendar}>
              ...
              : pending.leaveRequests.map((r,i)=>(
                  <div key={r.id} className="lm-row lm-in" style={{animationDelay:`${i*.05}s`}}>
                    <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:10}}>
                      <div>
                        <p style={{fontSize:12,color:'#64748b',marginBottom:2}}>Employee ID</p>
                      ...
                      {r.leaveType&&(
                        <span style={{padding:'3px 10px',borderRadius:999,background:'rgba(255,107,53,.08)',color:T.coral,border:'1px solid rgba(255,107,53,.2)',fontSize:10,fontWeight:700}}>
                          {r.leaveType}
                        </span>
                      )}
                    </div>
                    ...
                    {r.reason&&(
                      <div style={{padding:'8px 10px',borderRadius:9,background:'rgba(0,194,168,.05)',border:'1px solid rgba(0,194,168,.15)',marginBottom:10}}>
                        <p style={{fontSize:11,color:'#475569',lineHeight:1.65}}><strong style={{color:T.navy}}>Reason: </strong>{r.reason}</p>
                      </div>
                    )}

                    <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}>
                      <button className="lm-approve" onClick={()=>approveLeave(r.id)}><CheckCircle size={11}/>Approve</button>
                      <button className="lm-reject"  onClick={()=>rejectLeave(r.id)}><XCircle size={11}/>Reject</button>
                    </div>
                  </div>
                ))
            }
          ...
          {/* ── CORRECTION REQUESTS ── */}
          <SectionCard title="Pending Correction Requests" sub="Review attendance correction / regularization requests" count={pending.correctionRequests.length} accent={T.teal} Icon={Clock}>
              ...
              : pending.correctionRequests.map((r,i)=>(
                  <div key={r.id} className="lm-row lm-in" style={{animationDelay:`${i*.05}s`}}>
                    <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:10}}>
                      ...
                      <span style={{padding:'3px 10px',borderRadius:999,background:'rgba(0,194,168,.08)',color:T.teal,border:'1px solid rgba(0,194,168,.2)',fontSize:10,fontWeight:700}}>
                        Correction
                      </span>
                    </div>
