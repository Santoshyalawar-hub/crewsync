...
const DEPARTMENTS = [
  ...
  "Finance", "HR", "Operations", "Customer Success", "Legal", "Other",
];
...
.me-input:focus { border-color:${T.coral}; box-shadow:0 0 0 3px rgba(255,107,53,.1); }
...
.me-select:focus { border-color:${T.coral}; box-shadow:0 0 0 3px rgba(255,107,53,.1); }
...
function RoleBadge({ role }) {
  ...
  return (
    <span className="me-badge" style={{
      background: isAdmin ? "rgba(129,140,248,.1)" : "rgba(0,194,168,.1)",
      color:      isAdmin ? "#818cf8" : T.teal,
      border:     isAdmin ? "1px solid rgba(129,140,248,.3)" : "1px solid rgba(0,194,168,.3)",
    }}>
      {isAdmin ? <Shield size={9} style={{ marginRight: 4 }} /> : null}
      {role}
    </span>
  );
}
...
function Avatar({ name = "?", photoUrl }) {
  ...
  if (photoUrl) return <img src={photoUrl} alt={name} style={{ width: 36, height: 36, borderRadius: "50%", objectFit: "cover", border: "2px solid rgba(255,107,53,.2)" }} />;
  return (
    ...
    <div className="me" style={{ padding: "0 0 56px" }}>
      ...
      <div style={{ background: `linear-gradient(135deg,${T.navy},${T.navyMid})`, padding: "22px 26px", position: "relative", overflow: "hidden", marginBottom: 22 }}>
        ...
        <div style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div>
            <p style={{ fontSize: 11, fontWeight: 700, color: T.coral, textTransform: "uppercase", letterSpacing: ".12em", marginBottom: 4 }}>SamayaHR · People</p>
            <h1 className="fd" style={{ fontSize: 23, fontWeight: 900, color: "#fff", margin: 0 }}>Manage Employees</h1>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div style={{ display: "flex", background: "rgba(255,255,255,.1)", borderRadius: 10, padding: 3, gap: 2 }}>
              {[["ALL","All"],["EMPLOYEE","Employees"],["ADMIN","Admins"]].map(([val, lbl]) => (
            ...
            <button onClick={fetchEmployees} style={{ display: "flex", alignItems: "center", gap: 7, padding: "9px 16px", borderRadius: 11, border: "1px solid rgba(255,255,255,.2)", background: "rgba(255,255,255,.08)", color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "DM Sans" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: "#F8FAFF", borderBottom: `1.5px solid ${T.border}` }}>
                    {["Employee", "Department / Role", "Contact", "Joined", "Status", "Actions"].map(h => (
                      ...
                      <td style={{ padding: "12px 16px" }}>
                        <div style={{ display: "flex", gap: 6 }}>
                          <button className="me-btn" onClick={() => setViewEmp(emp)}
                            style={{ borderColor: "rgba(0,194,168,.3)", background: "rgba(0,194,168,.08)", color: T.teal }}>
                            <Eye size={11} />
                          </button>
                          ...
                          <button className="me-btn" onClick={() => deleteEmployee(emp)} disabled={deleting === emp.id}
        ...
        <Modal title={`Edit — ${editEmp.fullName}`} onClose={() => setEditEmp(null)} wide>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
            ...
            <button onClick={saveEdit} disabled={saving}
              style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "10px 20px", borderRadius: 11, background: `linear-gradient(135deg,${T.coral},#ff5722)`, color: "#fff", fontSize: 13, fontWeight: 800, border: "none", cursor: saving ? "not-allowed" : "pointer", opacity: saving ? .7 : 1, fontFamily: "DM Sans" }}>
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(13,31,45,.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 24 }}>
      <div className="me-in" style={{ background: "#fff", borderRadius: 20, width: "100%", maxWidth: wide ? 700 : 520, maxHeight: "90vh", overflow: "auto", boxShadow: "0 24px 64px rgba(13,31,45,.2)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 22px", borderBottom: `1.5px solid #E8ECF2` }}>
          <p style={{ fontSize: 16, fontWeight: 800, color: "#0D1F2D", fontFamily: "Sora,sans-serif" }}>{title}</p>
