...
/* ─────────────── ROLE META ─────────────── */
const ROLE_COLORS = {
  SUPER_ADMIN: { label: "Super Admin", color: "#FF6B35", bg: "rgba(255,107,53,.12)" },
  ADMIN:       { label: "Admin",       color: "#818cf8", bg: "rgba(129,140,248,.12)" },
  MANAGER:     { label: "Manager",     color: "#00C2A8", bg: "rgba(0,194,168,.12)" },
};
const roleMeta = (r = "") => {
  const key = r.toString().toUpperCase().replace(/\s+/g, "_");
  return ROLE_COLORS[key] || { label: r || "Admin", color: "#6b7280", bg: "#f3f4f6" };
};
const initials = (name = "") =>
...
const AdminManagement = ({ onBack }) => {
  ...
  const inputSt = (err) => ({
    ...
    border: `1.5px solid ${err ? "#fca5a5" : "#e5e7eb"}`, background: "#fff", color: "#0D1F2D",
  ...
  return (
    <>
      <style>{`
        ...
        .am-input:focus  { border-color: #FF6B35 !important; box-shadow: 0 0 0 3px rgba(255,107,53,.1); }
        .am-select:focus { border-color: #FF6B35 !important; box-shadow: 0 0 0 3px rgba(255,107,53,.1); }
        .am-row:hover { background: #fafafa; }
        .am-back:hover  { border-color: #FF6B35 !important; color: #FF6B35 !important; }
        .am-refresh:hover { border-color: #FF6B35 !important; color: #FF6B35 !important; }
        .am-overlay {
      ...
      <div className="am-root" style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 14, marginBottom: 26 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              <h1 className="am-sora" style={{ fontSize: 21, fontWeight: 900, color: "#0D1F2D", margin: 0, lineHeight: 1.2 }}>Admin Management</h1>
          ...
          <button onClick={() => { setShowModal(true); setSubmitError(""); setErrors({}); }}
            style={{ display: "flex", alignItems: "center", gap: 8, background: "linear-gradient(135deg,#FF6B35,#FF5722)", color: "#fff", border: "none", borderRadius: 12, padding: "10px 20px", fontSize: 13, fontWeight: 700, cursor: "pointer", boxShadow: "0 4px 14px rgba(255,107,53,.35)", transition: "all .2s", fontFamily: "'Sora',sans-serif" }}
            onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-1px)"; e.currentTarget.style.boxShadow = "0 8px 22px rgba(255,107,53,.45)"; }}
            onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "0 4px 14px rgba(255,107,53,.35)"; }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 22 }}>
          {[
            { label: "Total Admins",  val: admins.length, color: "#FF6B35" },
            { label: "Super Admins",  val: admins.filter(a => (a.adminRole||a.role||"").toUpperCase().includes("SUPER")).length, color: "#818cf8" },
            { label: "Managers",      val: admins.filter(a => (a.adminRole||a.role||"").toUpperCase().includes("MANAGER")).length, color: "#00C2A8" },
          ].map(s => (
        ...
        <div style={{ background: "#fff", borderRadius: 18, border: "1px solid #f0f0f0", boxShadow: "0 2px 18px rgba(13,31,45,.06)", overflow: "hidden" }}>
          ...
          <div style={{ padding: "18px 24px", borderBottom: "1px solid #f5f5f5", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#F7F8FA", border: "1.5px solid #e5e7eb", borderRadius: 10, padding: "8px 14px", flex: 1, maxWidth: 340 }}>
              <Ic d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" size={14} sw={2} />
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name or email…"
                style={{ border: "none", background: "transparent", outline: "none", fontSize: 13, color: "#0D1F2D", fontFamily: "'DM Sans',sans-serif", width: "100%" }} />
            </div>
            <button onClick={fetchAdmins} className="am-refresh"
              style={{ display: "flex", alignItems: "center", gap: 6, background: "#F7F8FA", border: "1.5px solid #e5e7eb", borderRadius: 10, padding: "8px 14px", fontSize: 12, fontWeight: 700, color: "#6b7280", cursor: "pointer", transition: "all .2s" }}>
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={4} style={{ padding: "64px 24px", textAlign: "center" }}>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#0D1F2D", marginBottom: 4 }}>
                        {search ? "No results found" : "No admins yet"}
                      </div>
                      <div style={{ fontSize: 12, color: "#9ca3af" }}>
                        {search ? "Try a different search term" : `Click "Add Admin" to get started`}
                      </div>
                    </td>
                  </tr>
                ) : filtered.map((admin, i) => {
                  const name = admin.name || admin.fullName || "—";
                  const role = admin.adminRole || admin.role || admin.position || "Admin";
                    ...
                    <tr key={admin.id || admin.adminId || admin.email || i} className="am-row"
                      ...
                      <td style={{ padding: "14px 24px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                          ...
                          <div>
                            <div style={{ fontSize: 13, fontWeight: 700, color: "#0D1F2D" }}>{name}</div>
      ...
      {showModal && (
        <div className="am-overlay" onClick={e => e.target === e.currentTarget && !submitting && setShowModal(false)}>
          <div style={{ background: "#fff", borderRadius: 20, width: "100%", maxWidth: 460, boxShadow: "0 28px 64px rgba(13,31,45,.28)", overflow: "hidden" }}>
            ...
            {/* modal header */}
            <div style={{ background: "linear-gradient(135deg,#0D1F2D,#162639)", padding: "22px 26px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                <h2 className="am-sora" style={{ fontSize: 17, fontWeight: 900, color: "#fff", margin: 0 }}>Add New Admin</h2>
              ...
              <button onClick={() => !submitting && setShowModal(false)}
                ...
                onMouseEnter={e => { e.currentTarget.style.background = "rgba(255,107,53,.3)"; e.currentTarget.style.color = "#fff"; }}
            ...
            <form onSubmit={handleSubmit} style={{ padding: "24px 26px", display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <Field label="Admin Name" error={errors.name}>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
                <button type="button" onClick={() => !submitting && setShowModal(false)} disabled={submitting}
                  ...
                  onMouseEnter={e => e.currentTarget.style.borderColor = "#0D1F2D"}
                  onMouseLeave={e => e.currentTarget.style.borderColor = "#e5e7eb"}>
                  Cancel
                </button>
                <button type="submit" disabled={submitting}
                  style={{ flex: 2, padding: "11px 0", borderRadius: 12, background: "linear-gradient(135deg,#FF6B35,#FF5722)", border: "none", fontSize: 13, fontWeight: 700, color: "#fff", cursor: "pointer", fontFamily: "'Sora',sans-serif", boxShadow: "0 4px 14px rgba(255,107,53,.35)", opacity: submitting ? 0.7 : 1, transition: "all .2s" }}>
