...
const EmptyState = ({ message }) => (
  <div style={{padding:'48px 20px',textAlign:'center',border:`1.5px dashed ${T.border}`,borderRadius:16,background:'#fafbff'}}>
    ...
    <p className="fd" style={{fontSize:14,fontWeight:800,color:T.navy,marginBottom:6}}>No Playbooks Yet</p>
    ...
    {p.fileUrl && (
      <div style={{display:'flex',gap:8,flexShrink:0}}>
        <a href={p.fileUrl} target="_blank" rel="noreferrer"
          style={{display:'inline-flex',alignItems:'center',gap:5,padding:'6px 11px',borderRadius:8,border:`1.5px solid rgba(6,182,212,.3)`,background:'rgba(6,182,212,.08)',color:T.teal,fontSize:11,fontWeight:700,textDecoration:'none'}}>
          <Eye size={10}/> View
        </a>
...
export default function PlaybookPage() {
  const [policyTab,      setPlaybookTab]      = useState("company");
  const [companyPlaybooks, setWorkspacePlaybooks] = useState([]);
  const [hrPlaybooks,      setHrPlaybooks]      = useState([]);
  ...
  useEffect(() => {
    ...
    api.get(`/api/policies/tenant/${tenantCode}`)
      .then(res => {
        ...
        setWorkspacePlaybooks(all.filter(p => !p.policyType || p.policyType === "COMPANY_POLICY"));
        setHrPlaybooks(all.filter(p => p.policyType === "PeopleOps_POLICY"));
      })
      .catch(() => { setWorkspacePlaybooks([]); setHrPlaybooks([]); })
      .finally(() => setLoading(false));
  }, [tenantCode]);

  const active = policyTab === "company" ? companyPlaybooks : hrPlaybooks;
    ...
    <div className="pv" style={{padding:'0 0 56px'}}>
      ...
      <div style={{background:`linear-gradient(135deg,${T.navy},${T.navyMid})`,padding:'22px 26px',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{position:'relative'}}>
          ...
          <h1 className="fd" style={{fontSize:23,fontWeight:900,color:'#fff',margin:0}}>Playbook Vault</h1>
      ...
      <div style={{padding:'14px 26px 0',display:'flex',gap:0}}>
        <div style={{display:'flex',background:'#fff',border:`1.5px solid ${T.border}`,borderRadius:12,padding:4,gap:3}}>
          {[["company","Workspace Playbooks"],["hr","PeopleOps Playbooks"]].map(([val,lbl]) => (
            <button key={val} onClick={() => setPlaybookTab(val)}
              style={{padding:'7px 18px',borderRadius:9,border:'none',fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:"'DM Sans',sans-serif",
                background: policyTab===val ? `linear-gradient(135deg,${T.coral},#06B6D4)` : 'transparent',
                color:      policyTab===val ? '#fff' : '#64748b',
                boxShadow:  policyTab===val ? '0 3px 10px rgba(139,92,246,.25)' : 'none',
                transition:'all .18s'}}>
              {lbl}
            </button>
          ))}
      ...
      <div style={{padding:'22px 26px',display:'flex',flexDirection:'column',gap:14}}>
        {loading ? (
          ...
          <>
            ...
            <div className="pv-panel pv-in">
              <div style={{background:`linear-gradient(90deg,${T.navy},${T.navyMid})`,padding:'12px 18px',display:'flex',alignItems:'center',gap:10}}>
                ...
                <p className="fd" style={{fontSize:13,fontWeight:800,color:'#fff'}}>
                  {policyTab==="company" ? "Workspace Playbooks" : "PeopleOps Playbooks"}
                </p>
                <span style={{marginLeft:'auto',padding:'2px 9px',borderRadius:999,
                  background: policyTab==="company" ? 'rgba(139,92,246,.2)' : 'rgba(99,102,241,.2)',
                ...
                {active.length === 0
                  ? <EmptyState message={
                      policyTab==="company"
                        ...
                        : "No PeopleOps policies have been published yet."
                    }/>
                  : active.map(p => <PlaybookCard key={p.id} p={p}/>)
                }
              </div>
            </div>
            ...
            {/* Info note */}
            {!tenantCode && (
              <div style={{padding:'14px 18px',borderRadius:12,background:'#FFFAF8',border:`1.5px solid rgba(139,92,246,.2)`}}>
